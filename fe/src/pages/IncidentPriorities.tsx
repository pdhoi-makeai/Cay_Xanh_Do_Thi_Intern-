...
export const IncidentPriorities: React.FC = () => {
  ...
  const fetchPriorities = async () => {
    setLoading(true);
    try {
      const list = await incidentService.getPrioritiesList(searchQuery || undefined);
      setPriorities(list);
    } catch (err) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setFormSuccess('');
    try {
      await incidentService.createPriority(selectedRecord);
      setFormSuccess(formMode === 'create' ? 'Thêm mới mức độ thành công!' : 'Cập nhật mức độ thành công!');
      setTimeout(() => {
        setIsDialogOpen(false);
        fetchPriorities();
      }, 1000);
    } catch (err: any) {
      setFormError(err.response?.data?.message || 'Không thể lưu mức độ sự cố.');
    }
  };

  const handleDelete = async (name: string) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa mức độ ${name}?`)) {
      try {
        await incidentService.deletePriority(name);
        fetchPriorities();
      } catch (err) {
        alert('Không thể xóa mức độ sự cố này.');
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-50 min-h-full">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-800 m-0">Mức độ sự cố</h2>
          <p className="text-sm text-slate-500 m-0">Thiết lập mức độ nghiêm trọng và khẩn cấp của sự cố</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm max-w-md w-full">
        <Input
          ...
          placeholder="Tìm theo tên mức độ..."
          value={searchQuery}
      ...
      <Dialog open={isDialogOpen} onOpenChange={(_, data) => setIsDialogOpen(data.open)}>
        <DialogSurface className="max-w-md w-full">
          <DialogBody>
            <DialogTitle>{formMode === 'create' ? 'Thêm mức độ mới' : 'Sửa thông tin mức độ'}</DialogTitle>
