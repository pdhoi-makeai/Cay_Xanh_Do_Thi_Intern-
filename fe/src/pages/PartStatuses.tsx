...
export const PartStatuses: React.FC = () => {
  const [statuses, setStatuses] = useState<PartRecord[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<Partial<PartRecord>>({});
  ...
  const fetchStatuses = async () => {
    setLoading(true);
    try {
      const list = await inspectionService.getPartStatusesList(searchQuery || undefined);
      setStatuses(list);
    } catch (err) {
      console.error('Failed to load part statuses', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenDialog = (mode: 'create' | 'edit', rec?: PartRecord) => {
    setFormMode(mode);
    setFormError('');
    setFormSuccess('');
    if (rec) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setFormSuccess('');
    try {
      await inspectionService.createPartStatus(selectedRecord);
      setFormSuccess(formMode === 'create' ? 'Thêm mới tình trạng bộ phận thành công!' : 'Cập nhật tình trạng bộ phận thành công!');
      setTimeout(() => {
        setIsDialogOpen(false);
        fetchStatuses();
      }, 1000);
    } catch (err: any) {
      setFormError(err.response?.data?.message || 'Không thể lưu tình trạng bộ phận cây.');
    }
  };

  const handleDelete = async (name: string) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa tình trạng bộ phận ${name}?`)) {
      try {
        await inspectionService.deletePartStatus(name);
        fetchStatuses();
      } catch (err) {
        alert('Không thể xóa tình trạng bộ phận này.');
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-50 min-h-full">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-800 m-0">Tình trạng bộ phận cây</h2>
          <p className="text-sm text-slate-500 m-0">Thiết lập các tình trạng bộ phận cụ thể của cây (Thân bình thường, cành khô héo, rễ nổi cản trở...)</p>
        </div>
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow>
                <TableHeaderCell className="font-bold text-slate-700">Tên tình trạng bộ phận</TableHeaderCell>
            ...
            <TableBody>
              {statuses.map((rec) => (
                <TableRow key={rec.name} className="hover:bg-slate-50/40">
                  <TableCell className="font-semibold text-slate-800">{rec.ten_tinh_trang}</TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(_, data) => setIsDialogOpen(data.open)}>
        <DialogSurface className="max-w-md w-full">
          <DialogBody>
            <DialogTitle>{formMode === 'create' ? 'Thêm tình trạng bộ phận mới' : 'Sửa tình trạng bộ phận'}</DialogTitle>
            <DialogContent className="py-4">
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <Label htmlFor="ten_tinh_trang_form" className="font-semibold text-gray-700">Tên tình trạng *</Label>
                  <Input
                    id="ten_tinh_trang_form"
                    required
                    disabled={formMode === 'edit'}
                    value={selectedRecord.ten_tinh_trang || ''}
                    onChange={(e) => setSelectedRecord({ ...selectedRecord, ten_tinh_trang: e.target.value })}
                  />
                </div>
