...
export const LeaningStatuses: React.FC = () => {
  const [statuses, setStatuses] = useState<LeaningRecord[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<Partial<LeaningRecord>>({});
  ...
  const fetchStatuses = async () => {
    setLoading(true);
    try {
      const list = await inspectionService.getLeaningStatusesList(searchQuery || undefined);
      setStatuses(list);
    } catch (err) {
      console.error('Failed to load leaning statuses', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenDialog = (mode: 'create' | 'edit', rec?: LeaningRecord) => {
    setFormMode(mode);
    setFormError('');
    setFormSuccess('');
    if (rec) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setFormSuccess('');
    try {
      await inspectionService.createLeaningStatus(selectedRecord);
      setFormSuccess(formMode === 'create' ? 'Thêm mới trạng thái nghiêng thành công!' : 'Cập nhật trạng thái nghiêng thành công!');
      setTimeout(() => {
        setIsDialogOpen(false);
        fetchStatuses();
      }, 1000);
    } catch (err: any) {
      setFormError(err.response?.data?.message || 'Không thể lưu trạng thái nghiêng.');
    }
  };

  const handleDelete = async (name: string) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa trạng thái nghiêng ${name}?`)) {
      try {
        await inspectionService.deleteLeaningStatus(name);
        fetchStatuses();
      } catch (err) {
        alert('Không thể xóa trạng thái nghiêng này.');
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-50 min-h-full">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-800 m-0">Trạng thái nghiêng</h2>
          <p className="text-sm text-slate-500 m-0">Thiết lập các mức độ nghiêng của cây (Không nghiêng, nghiêng nhẹ, nguy cơ đổ ngã...)</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm max-w-md w-full">
        <Input
          ...
          placeholder="Tìm theo tên trạng thái nghiêng..."
          value={searchQuery}
      ...
      <Dialog open={isDialogOpen} onOpenChange={(_, data) => setIsDialogOpen(data.open)}>
        <DialogSurface className="max-w-md w-full">
          <DialogBody>
            <DialogTitle>{formMode === 'create' ? 'Thêm trạng thái nghiêng mới' : 'Sửa trạng thái nghiêng'}</DialogTitle>
