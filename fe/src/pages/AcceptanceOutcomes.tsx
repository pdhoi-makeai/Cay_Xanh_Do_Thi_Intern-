...
import { acceptanceService } from '../services/api';
import { 
  Button, 
  Table, 
  TableHeader, 
...
export const AcceptanceOutcomes: React.FC = () => {
  const [types, setTypes] = useState<OutcomeRecord[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<Partial<OutcomeRecord>>({});
  ...
  const fetchTypes = async () => {
    setLoading(true);
    try {
      const list = await acceptanceService.getResultsList(searchQuery || undefined);
      setTypes(list);
    } catch (err) {
      console.error('Failed to load acceptance outcomes', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenDialog = (mode: 'create' | 'edit', rec?: OutcomeRecord) => {
    setFormMode(mode);
    setFormError('');
    setFormSuccess('');
    if (rec) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setFormSuccess('');
    try {
      await acceptanceService.createResult(selectedRecord);
      setFormSuccess(formMode === 'create' ? 'Thêm mới kết quả thành công!' : 'Cập nhật kết quả thành công!');
      setTimeout(() => {
        setIsDialogOpen(false);
        fetchTypes();
      }, 1000);
    } catch (err: any) {
      setFormError(err.response?.data?.message || 'Không thể lưu kết quả.');
    }
  };

  const handleDelete = async (name: string) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa kết quả ${name}?`)) {
      try {
        await acceptanceService.deleteResult(name);
        fetchTypes();
      } catch (err) {
        alert('Không thể xóa kết quả này.');
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-50 min-h-full">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-800 m-0">Kết quả nghiệm thu</h2>
          <p className="text-sm text-slate-500 m-0">Thiết lập danh mục kết quả nghiệm thu (Đạt, Đạt có điều kiện, Không đạt...)</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm max-w-md w-full">
        <Input
          ...
          placeholder="Tìm theo tên kết quả..."
          value={searchQuery}
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow>
                <TableHeaderCell className="font-bold text-slate-700">Tên kết quả</TableHeaderCell>
            ...
            <TableBody>
              {types.map((rec) => (
                <TableRow key={rec.name} className="hover:bg-slate-50/40">
                  <TableCell className="font-semibold text-slate-800">{rec.ten_ket_qua}</TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(_, data) => setIsDialogOpen(data.open)}>
        <DialogSurface className="max-w-md w-full">
          <DialogBody>
            <DialogTitle>{formMode === 'create' ? 'Thêm kết quả mới' : 'Sửa thông tin kết quả'}</DialogTitle>
            <DialogContent className="py-4">
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <Label htmlFor="ten_ket_qua_form" className="font-semibold text-gray-700">Tên kết quả *</Label>
                  <Input
                    id="ten_ket_qua_form"
                    required
                    disabled={formMode === 'edit'}
                    value={selectedRecord.ten_ket_qua || ''}
                    onChange={(e) => setSelectedRecord({ ...selectedRecord, ten_ket_qua: e.target.value })}
                  />
                </div>
