...
export const ActionTypes: React.FC = () => {
  const [types, setTypes] = useState<ActionTypeRecord[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<Partial<ActionTypeRecord>>({});
  ...
  const fetchTypes = async () => {
    setLoading(true);
    try {
      const list = await executionService.getActionTypesList(searchQuery || undefined);
      setTypes(list);
    } catch (err) {
  ...
  const handleOpenDialog = (mode: 'create' | 'edit', rec?: ActionTypeRecord) => {
    setFormMode(mode);
    setFormError('');
    setFormSuccess('');
    if (rec) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setFormSuccess('');
    try {
      await executionService.createActionType(selectedRecord);
      setFormSuccess(formMode === 'create' ? 'Thêm mới loại hành động thành công!' : 'Cập nhật loại hành động thành công!');
      setTimeout(() => {
        setIsDialogOpen(false);
        fetchTypes();
      }, 1000);
    } catch (err: any) {
      setFormError(err.response?.data?.message || 'Không thể lưu loại hành động.');
    }
  };

  const handleDelete = async (name: string) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa loại hành động ${name}?`)) {
      try {
        await executionService.deleteActionType(name);
        fetchTypes();
      } catch (err) {
        alert('Không thể xóa loại hành động này.');
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-50 min-h-full">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-800 m-0">Loại hành động</h2>
          <p className="text-sm text-slate-500 m-0">Thiết lập danh mục các loại hành động thi công (Thu gom cành, tỉa cành, đốn hạ, tưới nước...)</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm max-w-md w-full">
        <Input
          ...
          placeholder="Tìm theo tên loại hành động..."
          value={searchQuery}
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow>
                <TableHeaderCell className="font-bold text-slate-700">Tên loại hành động</TableHeaderCell>
            ...
            <TableBody>
              {types.map((rec) => (
                <TableRow key={rec.name} className="hover:bg-slate-50/40">
                  <TableCell className="font-semibold text-slate-800">{rec.ten_hanh_dong}</TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(_, data) => setIsDialogOpen(data.open)}>
        <DialogSurface className="max-w-md w-full">
          <DialogBody>
            <DialogTitle>{formMode === 'create' ? 'Thêm loại hành động mới' : 'Sửa thông tin loại hành động'}</DialogTitle>
            <DialogContent className="py-4">
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <Label htmlFor="ten_hanh_dong_form" className="font-semibold text-gray-700">Tên loại hành động *</Label>
                  <Input
                    id="ten_hanh_dong_form"
                    required
                    disabled={formMode === 'edit'}
                    value={selectedRecord.ten_hanh_dong || ''}
                    onChange={(e) => setSelectedRecord({ ...selectedRecord, ten_hanh_dong: e.target.value })}
                  />
                </div>
