...
export const IncidentTypes: React.FC = () => {
  ...
  const fetchTypes = async () => {
    setLoading(true);
    try {
      const list = await incidentService.getTypesList(searchQuery || undefined);
      setTypes(list);
    } catch (err) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setFormSuccess('');
    try {
      await incidentService.createType(selectedRecord);
      setFormSuccess(formMode === 'create' ? 'Thêm mới loại sự cố thành công!' : 'Cập nhật loại sự cố thành công!');
      setTimeout(() => {
        setIsDialogOpen(false);
        fetchTypes();
      }, 1000);
    } catch (err: any) {
      setFormError(err.response?.data?.message || 'Không thể lưu loại sự cố.');
    }
  };

  const handleDelete = async (name: string) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa loại sự cố ${name}?`)) {
      try {
        await incidentService.deleteType(name);
        fetchTypes();
      } catch (err) {
        alert('Không thể xóa loại sự cố này.');
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-50 min-h-full">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-800 m-0">Loại sự cố</h2>
          <p className="text-sm text-slate-500 m-0">Thiết lập các phân loại sự cố (Cây nghiêng, gãy cành, vướng đường điện...)</p>
        </div>
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow>
                <TableHeaderCell className="font-bold text-slate-700">Tên loại</TableHeaderCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(_, data) => setIsDialogOpen(data.open)}>
        <DialogSurface className="max-w-md w-full">
          <DialogBody>
            <DialogTitle>{formMode === 'create' ? 'Thêm loại sự cố mới' : 'Sửa thông tin loại sự cố'}</DialogTitle>
            <DialogContent className="py-4">
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <Label htmlFor="ten_loai_form" className="font-semibold text-gray-700">Tên loại *</Label>
                  <Input
                    id="ten_loai_form"
                    required
