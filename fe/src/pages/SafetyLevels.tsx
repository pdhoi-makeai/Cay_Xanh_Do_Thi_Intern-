...
export const SafetyLevels: React.FC = () => {
  const [levels, setLevels] = useState<SafetyRecord[]>([]);
  ...
  const [selectedRecord, setSelectedRecord] = useState<Partial<SafetyRecord>>({});
  ...
  const fetchLevels = async () => {
    setLoading(true);
    try {
      const list = await inspectionService.getSafetyLevelsList(searchQuery || undefined);
      setLevels(list);
    } catch (err) {
      console.error('Failed to load safety levels', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenDialog = (mode: 'create' | 'edit', rec?: SafetyRecord) => {
    setFormMode(mode);
    setFormError('');
    setFormSuccess('');
    if (rec) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setFormSuccess('');
    try {
      await inspectionService.createSafetyLevel(selectedRecord);
      setFormSuccess(formMode === 'create' ? 'Thêm mới mức an toàn thành công!' : 'Cập nhật mức an toàn thành công!');
      setTimeout(() => {
        setIsDialogOpen(false);
        fetchLevels();
      }, 1000);
    } catch (err: any) {
      setFormError(err.response?.data?.message || 'Không thể lưu mức an toàn.');
    }
  };

  const handleDelete = async (name: string) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa mức an toàn ${name}?`)) {
      try {
        await inspectionService.deleteSafetyLevel(name);
        fetchLevels();
      } catch (err) {
        alert('Không thể xóa mức an toàn này.');
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-50 min-h-full">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-800 m-0">Mức an toàn</h2>
          <p className="text-sm text-slate-500 m-0">Thiết lập các mức độ đánh giá độ an toàn (An toàn, cần theo dõi, nguy hiểm...)</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm max-w-md w-full">
        <Input
          ...
          placeholder="Tìm theo tên mức an toàn..."
          value={searchQuery}
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow>
                <TableHeaderCell className="font-bold text-slate-700">Tên mức độ</TableHeaderCell>
            ...
            <TableBody>
              {levels.map((rec) => (
                <TableRow key={rec.name} className="hover:bg-slate-50/40">
                  <TableCell className="font-semibold text-slate-800">{rec.ten_muc_do}</TableCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(_, data) => setIsDialogOpen(data.open)}>
        <DialogSurface className="max-w-md w-full">
          <DialogBody>
            <DialogTitle>{formMode === 'create' ? 'Thêm mức an toàn mới' : 'Sửa mức an toàn'}</DialogTitle>
            <DialogContent className="py-4">
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <Label htmlFor="ten_muc_do_form" className="font-semibold text-gray-700">Tên mức độ *</Label>
                  <Input
                    id="ten_muc_do_form"
                    required
                    disabled={formMode === 'edit'}
                    value={selectedRecord.ten_muc_do || ''}
                    onChange={(e) => setSelectedRecord({ ...selectedRecord, ten_muc_do: e.target.value })}
                  />
                </div>
