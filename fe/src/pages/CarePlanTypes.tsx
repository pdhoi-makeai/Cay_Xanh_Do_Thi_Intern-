...
export const CarePlanTypes: React.FC = () => {
  ...
  const fetchTypes = async () => {
    setLoading(true);
    try {
      const list = await carePlanService.getPlanTypesList(searchQuery || undefined);
      setTypes(list);
    } catch (err) {
      console.error('Failed to load care plan types', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setFormSuccess('');
    try {
      await carePlanService.createPlanType(selectedRecord);
      setFormSuccess(formMode === 'create' ? 'Thêm mới loại kế hoạch thành công!' : 'Cập nhật loại kế hoạch thành công!');
      setTimeout(() => {
        setIsDialogOpen(false);
        fetchTypes();
      }, 1000);
    } catch (err: any) {
      setFormError(err.response?.data?.message || 'Không thể lưu loại kế hoạch.');
    }
  };

  const handleDelete = async (name: string) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa loại kế hoạch ${name}?`)) {
      try {
        await carePlanService.deletePlanType(name);
        fetchTypes();
      } catch (err) {
        alert('Không thể xóa loại kế hoạch này.');
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-50 min-h-full">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-800 m-0">Loại kế hoạch</h2>
          <p className="text-sm text-slate-500 m-0">Thiết lập loại hình kế hoạch chăm sóc (Xử lý cây nguy hiểm, cắt tỉa định kỳ, bón phân...)</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm max-w-md w-full">
        <Input
          ...
          placeholder="Tìm theo tên loại kế hoạch..."
          value={searchQuery}
        ...
        {loading ? (
          ...
          <Table>
            <TableHeader className="bg-slate-50/50">
              <TableRow>
                <TableHeaderCell className="font-bold text-slate-700">Tên loại kế hoạch</TableHeaderCell>
      ...
      <Dialog open={isDialogOpen} onOpenChange={(_, data) => setIsDialogOpen(data.open)}>
        <DialogSurface className="max-w-md w-full">
          <DialogBody>
            <DialogTitle>{formMode === 'create' ? 'Thêm loại kế hoạch mới' : 'Sửa thông tin loại kế hoạch'}</DialogTitle>
            <DialogContent className="py-4">
              <form onSubmit={handleSave} className="flex flex-col gap-4">
                ...
                <div className="flex flex-col gap-1">
                  <Label htmlFor="ten_loai_form" className="font-semibold text-gray-700">Tên loại kế hoạch *</Label>
                  <Input
                    id="ten_loai_form"
                    required
