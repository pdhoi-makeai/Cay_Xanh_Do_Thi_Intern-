...
export const CarePlanPriorities: React.FC = () => {
  ...
  const [selectedRecord, setSelectedRecord] = useState<Partial<PriorityRecord>>({});
  ...
  const fetchPriorities = async () => {
    setLoading(true);
    try {
      const list = await carePlanService.getPlanPrioritiesList(searchQuery || undefined);
      setPriorities(list);
    } catch (err) {
      console.error('Failed to load care plan priorities', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleOpenDialog = (mode: 'create' | 'edit', rec?: PriorityRecord) => {
    setFormMode(mode);
    setFormError('');
    setFormSuccess('');
    if (rec) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    setFormSuccess('');
    try {
      await carePlanService.createPlanPriority(selectedRecord);
      setFormSuccess(formMode === 'create' ? 'Thêm mới mức ưu tiên thành công!' : 'Cập nhật mức ưu tiên thành công!');
      setTimeout(() => {
        setIsDialogOpen(false);
        fetchPriorities();
      }, 1000);
    } catch (err: any) {
      setFormError(err.response?.data?.message || 'Không thể lưu mức ưu tiên.');
    }
  };

  const handleDelete = async (name: string) => {
    if (window.confirm(`Bạn có chắc chắn muốn xóa mức ưu tiên ${name}?`)) {
      try {
        await carePlanService.deletePlanPriority(name);
        fetchPriorities();
      } catch (err) {
        alert('Không thể xóa mức ưu tiên này.');
      }
    }
  };
  ...
  return (
    <div className="flex flex-col gap-6 bg-slate-50 min-h-full">
      ...
      <div className="flex justify-between items-center bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm">
        <div>
          <h2 className="text-xl font-bold text-slate-800 m-0">Mức ưu tiên</h2>
          <p className="text-sm text-slate-500 m-0">Thiết lập các mức độ ưu tiên của kế hoạch chăm sóc (Cao, Trung bình, Thấp...)</p>
        </div>
      ...
      <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-sm max-w-md w-full">
        <Input
          ...
          placeholder="Tìm theo tên mức ưu tiên..."
          value={searchQuery}
      ...
      <Dialog open={isDialogOpen} onOpenChange={(_, data) => setIsDialogOpen(data.open)}>
        <DialogSurface className="max-w-md w-full">
          <DialogBody>
            <DialogTitle>{formMode === 'create' ? 'Thêm mức ưu tiên mới' : 'Sửa mức ưu tiên'}</DialogTitle>
